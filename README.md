...
1. Create a new Ubuntu Linux server instance using *Amazon Lightsail*
Referrence : [Amazon Lightsail](https://lightsail.aws.amazon.com/ls/webapp/home/)
...
Use command `Sudo nano /etc/ssh/sshd_config` to change the port number as desired.
Also in the same file you can change values for following variables:
`Permitrootlogin no` to disable access to root login 
and `Passwordauthentication no` to allow key-based authentication.
...
`Sudo ufw status` *initially it is inactive*
...
`Sudo ufw allow ntp`
`Sudo ufw enable`
`Sudo ufw status` *now it will show active status with managed ports*
...
7. Give grader the permission to sudo.
   In ubuntu, you can copy and edit the user's file in *sudoers* directory.
`sudo /etc/sudoers.d/existing_user_name /etc/sudoers.d/grader`
then use `sudo nano /etc/sudoers.d/grader` and make changes to *username* as grader. Save the file and exit.
...
Then it will create few files and save the details.
Then use `cat ~/.ssh/grader_rsa.pub` and copy these contents in order to save further.
After generating a key switch to *grader* user and do the followng:
`mkdir .ssh` (if dirctory is not there)
`touch .ssh/authorized_keys`
`nano .ssh/authorized_keys` and paste the above copied contents in it.
now use `chmod 700 .ssh` and `chmod 644 .ssh/authorized_keys` and we are set to use key based authentication.
Further on client machine you can use following to logged in with *grader*
...
Use command `sudo -i -u postgres psql` to start editing in psql mode with postgres user.
Use command `CREATE ROLE catalog WITH LOGIN PASSWORD '*****';` to create a user with some password string.
Further you can use command `\du` to list out the users and their roles.
Once you are done, exit the psql.
c. Use command `sudo service postgresql restart` to restart the postgresql
d. After these setting, it is reccomended to restart the *apache2 server*
...
Further you can oncfigure it by providing basic information such as username, email.
`git config --global user.name "Example User"`
`git config --global user.email email@example.com`

13. Clone and setup your project from the Github repository. Here we are using *Item-Catalog-Project* from GitHub repository.
a. Enable mod_wsgi.
...
(3) Move contents of application.py to __init__.py
(4) Get the hostname for your IP at [hcidata](http://www.hcidata.info/host2ip.cgi)
...
Download the client_secrets.json newly created. And update the hostname, ip etc. in Ubuntu also accordingly.
(b) For Facebook authentication:
Go to [Facebook Developer Console](https://developers.facebook.com/)
Then go to your app, further go to facebook login and open settings.
Update the valid OAuth redirect URIs with our hostname and url.
Update the fb_client_secrets.json file from terminal also.
Make sure for the updated values in both the client_secrets.json files.
Restart Apache server : `sudo service apache2 restart`
(4) Execute __init.py__ 
It will show message like "Running on ...."
Then you can enter URL from browser to run the project.
Referrences :
(1) [hcidata to get a hostname](http://www.hcidata.info/host2ip.cgi)
...
(4) [Udacity Discussion Forums](https://discussions.udacity.com/t/setting-facebook-valid-oauth-redirect-uris/160617)
